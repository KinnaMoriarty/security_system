<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>代办项目</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片视图区域 -->
    <el-card>
      <!-- 项目列表区 -->
      <el-table :data="relativeProject" border stripe>
        ...
        <el-table-column label="报表状态">
          <template slot-scope="scope">
            <!-- 报表提交【2】/未提交【1】：reportStatus -->
            <el-tag :type="isHandled(scope.row)">
              {{
                relativeProject[scope.$index].status == 1
                  ? "未提交"
                  : relativeProject[scope.$index].status == 2
                  ? "已提交待审核"
                  : "未通过审核"
              }}</el-tag
            >
        ...
        <el-table-column label="查看">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              icon="el-icon-search"
              size="mini"
              @click="showProjectDetail(scope.$index)"
            ></el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    >
      ...
      >
        ...
        <el-descriptions-item>
          ...
          <!-- 当没有上传时 -->
          <el-upload
            v-show="!reportIsHandled"
            class="upload-demo"
            ref="upload"
            :data="{ projectID: projectDetail.projectID }"
            multiple:false
            action="http://192.168.0.110:8080/SecurityManagementSystem/ProjectServlet?action=uploadFile"
...
export default {
  ...
  methods: {
    ...
    async getProjectList() {
      let requestParams = `/projectServlet?action=getProjects&workerID=${this.$root.nowLoginId}&status=1`;
      ...
      // 项目信息
      this.relativeProject = res.projects;
      for (let i = 0; i < res.projects.length; i++) {
        this.relativeProject[i].status =
          res.reportStatus[this.relativeProject[i].projectID];
      }
    },
    // 2.判断报表状态
    isHandled(row) {
    ...
    async showProjectDetail(index) {
      let requestParams = `/projectServlet?action=getAllOfProject&projectID=${this.relativeProject[index].projectID}`;
      let requestParams1 = `/projectServlet?action=getReport&projectID=${this.relativeProject[index].projectID}`;
      let report;

      if (this.relativeProject[index].status == 1) {
        this.reportIsHandled = false;
      }
      const { data: res } = await this.$http.post(requestParams);
      if (this.relativeProject[index].status == 2) {
